...
void cMeshRenderSystem::update(double deltaTime) {
	...
	// Draw current scene
	this->drawScene_Simple(this->entitiesToUpdate, 0,deltaTime);
...
void cMeshRenderSystem::drawSkyBox() {
	...
	this->drawObject(pSkyBox, matIdentity, 0.0f);
...
void cMeshRenderSystem::drawObject(cEntity* pEntity, glm::mat4x4 &matModel, float deltaTime) {
	...
	if (!pEntity->hasComponent<cSkinnedMeshComponent>()) {
		...
		modelInfo.meshFileName = skinMeshComp->meshName;
		...
		runningAnimation.incrementTime(deltaTime);
		skinMeshComp->boneTransform(runningAnimation.elapsedTime, //curFrameTime
...
void cMeshRenderSystem::drawObjectInstances(cEntity* pCurrentMesh) {
	...
	glUniformMatrix4fv(program->getUniformID_From_Name("view"), 1, GL_FALSE, glm::value_ptr(matView));
	glUniformMatrix4fv(program->getUniformID_From_Name("projection"), 1, GL_FALSE, glm::value_ptr(matProjection));

	std::vector<cInstancedMeshComponent::sDrawInfo> instances = pCurrentMesh->getComponent<cInstancedMeshComponent>()->generateDrawInfo();
	for (size_t index = 0; index < instances.size(); ++index) {

		this->instanceRenderer.drawInstances(this->instanceRenderer.generateVBO(instances[index]));
	}

	return;
}

void cMeshRenderSystem::drawScene_Simple(std::vector<cEntity*> pEntities, unsigned int passNumber, float deltaTime) {
	...
	for (unsigned int objIndex = 0; objIndex != pEntities.size(); objIndex++) {
		...
		if (pEntities[objIndex]->getComponent<cRenderMeshComponent>()->materialDiffuse.a >= 1.0f) {
			this->drawObject(pEntities[objIndex], matModel, deltaTime);
		}
		else {
	...
	for (unsigned int index = 0; index != vec_transparentObj.size(); ++index) {
		...
		glm::mat4x4 matModel = glm::mat4(1.0f);
		this->drawObject(vec_transparentObj[index], matModel, deltaTime);
	}
