...
cScene::cScene() : cameraUp(UP), selectedEntity(0), selectedLight(0), selectedChannelGroup(0) {
	...
	this->bFollowSelectedEntity = false;
...
void cScene::setSelectedEntity(cEntity* pEntity) {

	std::vector<cEntity*>::iterator it = std::find(this->entities.begin(), this->entities.end(), pEntity);

	if (it != this->entities.end()) {

		this->selectedEntity = std::distance(this->entities.begin(), it);
	}
...
void cScene::changeSelectedRigidBody(bool next) {
	...
	//make sure it is a Skin Mesh body
	if (entities[this->selectedEntity]->hasComponent<cSkinnedMeshComponent>() == false) {
...
void cScene::setLightDebugSphere(cEntity* pSphere) {
	...
	cMeshRenderSystem::getInstance()->drawObject(pSphere, matBall, 0.0f);
