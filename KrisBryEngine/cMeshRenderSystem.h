...
public:
	...
	void drawSkyBox();
	void drawObject(cEntity* pCurrentMesh, glm::mat4x4 &matModel, float deltaTime);
	void drawObjectInstances(cEntity* pCurrentMesh);
	void drawScene_Simple(std::vector<cEntity*> pCurrentMesh, unsigned int passNumber, float deltaTime);

	cInstancedSystem instanceRenderer;
