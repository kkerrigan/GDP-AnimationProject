...
bool cSerialization::deserializeBulletConstraints(std::string fileName) {
	...
	assert(document.HasMember("bulletConstraints"));
	assert(document["bulletConstraints"].IsArray());

	rapidjson::GenericArray < false, rapidjson::Value::ValueType> bulletConstraints = (document["bulletConstraints"].GetArray());
	for (rapidjson::Value::ConstValueIterator itr = bulletConstraints.Begin(); itr != bulletConstraints.End(); ++itr) {

		nPhysics::iPhysicsFactory* physicsFactory = cEngine::getInstance()->getPhysicsFactory();
		nPhysics::iPhysicsWorld* physicsWorld = cEngine::getInstance()->getPhysicsWorld();

		assert(itr->HasMember("constraintType"));
		assert((*itr)["constraintType"].IsString());
		std::string constraintType = (*itr)["constraintType"].GetString();

		if (constraintType == "hinge") {

			nPhysics::sHingeConstraintDef hingeDef;

			assert(itr->HasMember("rigidBodyAName"));
			assert((*itr)["rigidBodyAName"].IsString());
			std::string rbAName = (*itr)["rigidBodyAName"].GetString();

			assert(itr->HasMember("rigidBodyBName"));
			assert((*itr)["rigidBodyBName"].IsString());
			std::string rbBName = (*itr)["rigidBodyBName"].GetString();

			// Get the rigid bodies
			hingeDef.rigidBodyA = pScene->findObjectByFriendlyName(rbAName)->getComponent<cRigidBodyComponent>()->getRigidBody();
			hingeDef.rigidBodyB = pScene->findObjectByFriendlyName(rbBName)->getComponent<cRigidBodyComponent>()->getRigidBody();

			glm::vec3 pivotA(0.0f);
			assert(itr->HasMember("pivotInA"));
			assert((*itr)["pivotInA"].IsObject());

			assert((*itr)["pivotInA"].HasMember("x"));
			assert((*itr)["pivotInA"]["x"].IsDouble());
			pivotA.x = (float)(*itr)["pivotInA"]["x"].GetDouble();

			assert((*itr)["pivotInA"].HasMember("y"));
			assert((*itr)["pivotInA"]["y"].IsDouble());
			pivotA.y = (float)(*itr)["pivotInA"]["y"].GetDouble();

			assert((*itr)["pivotInA"].HasMember("z"));
			assert((*itr)["pivotInA"]["z"].IsDouble());
			pivotA.z = (float)(*itr)["pivotInA"]["z"].GetDouble();

			glm::vec3 pivotB(0.0f);
			assert(itr->HasMember("pivotInB"));
			assert((*itr)["pivotInB"].IsObject());

			assert((*itr)["pivotInB"].HasMember("x"));
			assert((*itr)["pivotInB"]["x"].IsDouble());
			pivotB.x = (float)(*itr)["pivotInB"]["x"].GetDouble();

			assert((*itr)["pivotInB"].HasMember("y"));
			assert((*itr)["pivotInB"]["y"].IsDouble());
			pivotB.y = (float)(*itr)["pivotInB"]["y"].GetDouble();

			assert((*itr)["pivotInB"].HasMember("z"));
			assert((*itr)["pivotInB"]["z"].IsDouble());
			pivotB.z = (float)(*itr)["pivotInB"]["z"].GetDouble();

			glm::vec3 axisA(0.0f);
			assert(itr->HasMember("axisInA"));
			assert((*itr)["axisInA"].IsObject());

			assert((*itr)["axisInA"].HasMember("x"));
			assert((*itr)["axisInA"]["x"].IsDouble());
			axisA.x = (float)(*itr)["axisInA"]["x"].GetDouble();

			assert((*itr)["axisInA"].HasMember("y"));
			assert((*itr)["axisInA"]["y"].IsDouble());
			axisA.y = (float)(*itr)["axisInA"]["y"].GetDouble();

			assert((*itr)["axisInA"].HasMember("z"));
			assert((*itr)["axisInA"]["z"].IsDouble());
			axisA.z = (float)(*itr)["axisInA"]["z"].GetDouble();

			glm::vec3 axisB(0.0f);
			assert(itr->HasMember("axisInB"));
			assert((*itr)["axisInB"].IsObject());

			assert((*itr)["axisInB"].HasMember("x"));
			assert((*itr)["axisInB"]["x"].IsDouble());
			axisB.x = (float)(*itr)["axisInB"]["x"].GetDouble();

			assert((*itr)["axisInB"].HasMember("y"));
			assert((*itr)["axisInB"]["y"].IsDouble());
			axisB.y = (float)(*itr)["axisInB"]["y"].GetDouble();

			assert((*itr)["axisInB"].HasMember("z"));
			assert((*itr)["axisInB"]["z"].IsDouble());
			axisB.z = (float)(*itr)["axisInB"]["z"].GetDouble();

			// Set the rest of the def and create the constraint
			hingeDef.pivotInA = pivotA;
			hingeDef.pivotInB = pivotB;
			hingeDef.axisInA = axisA;
			hingeDef.axisInB = axisB;

			nPhysics::iConstraint* hingeConstraint = physicsFactory->createHingeConstraint(hingeDef);
			physicsWorld->addConstraint(hingeConstraint);
		}
		else if (constraintType == "ballSocket") {

			nPhysics::sBallSocketConstraintDef ballSocketDef;
			...
			// Get the rigid bodies
			ballSocketDef.rigidBodyA = pScene->findObjectByFriendlyName(rbAName)->getComponent<cRigidBodyComponent>()->getRigidBody();
			ballSocketDef.rigidBodyB = pScene->findObjectByFriendlyName(rbBName)->getComponent<cRigidBodyComponent>()->getRigidBody();
			...
			// Set the rest of the def and create the constraint
			ballSocketDef.pivotInA = pivotA;
			ballSocketDef.pivotInB = pivotB;

			nPhysics::iConstraint* ballSocketConstraint = physicsFactory->createBallSocketConstraint(ballSocketDef);
			physicsWorld->addConstraint(ballSocketConstraint);
		}
		else if (constraintType == "slider") {

			nPhysics::sSliderConstraintDef sliderDef;
			...
			// Get the rigid bodies
			sliderDef.rigidBodyA = pScene->findObjectByFriendlyName(rbAName)->getComponent<cRigidBodyComponent>()->getRigidBody();

			cEntity* bodyB = nullptr;
			bodyB = pScene->findObjectByFriendlyName(rbBName);
			if (bodyB) {

				sliderDef.rigidBodyB = bodyB->getComponent<cRigidBodyComponent>()->getRigidBody();
			}

			glm::vec3 frameA(0.0f);
			assert(itr->HasMember("frameAEuler"));
			assert((*itr)["frameAEuler"].IsObject());

			assert((*itr)["frameAEuler"].HasMember("x"));
			assert((*itr)["frameAEuler"]["x"].IsDouble());
			frameA.x = (float)(*itr)["frameAEuler"]["x"].GetDouble();

			assert((*itr)["frameAEuler"].HasMember("y"));
			assert((*itr)["frameAEuler"]["y"].IsDouble());
			frameA.y = (float)(*itr)["frameAEuler"]["y"].GetDouble();

			assert((*itr)["frameAEuler"].HasMember("z"));
			assert((*itr)["frameAEuler"]["z"].IsDouble());
			frameA.z = (float)(*itr)["frameAEuler"]["z"].GetDouble();

			glm::vec3 frameB(0.0f);
			assert(itr->HasMember("frameBEuler"));
			assert((*itr)["frameBEuler"].IsObject());

			assert((*itr)["frameBEuler"].HasMember("x"));
			assert((*itr)["frameBEuler"]["x"].IsDouble());
			frameB.x = (float)(*itr)["frameBEuler"]["x"].GetDouble();

			assert((*itr)["frameBEuler"].HasMember("y"));
			assert((*itr)["frameBEuler"]["y"].IsDouble());
			frameB.y = (float)(*itr)["frameBEuler"]["y"].GetDouble();

			assert((*itr)["frameBEuler"].HasMember("z"));
			assert((*itr)["frameBEuler"]["z"].IsDouble());
			frameB.z = (float)(*itr)["frameBEuler"]["z"].GetDouble();

			bool bUseLinearFrame = false;
			assert(itr->HasMember("useLinearRefFrame"));
			assert((*itr)["useLinearRefFrame"].IsBool());
			bUseLinearFrame = (*itr)["useLinearRefFrame"].GetBool();

			// Set the rest of the def and create the constraint
			sliderDef.frameAEuler = frameA;
			sliderDef.frameBEuler = frameB;
			sliderDef.bUseLinearReferenceFrameA = bUseLinearFrame;

			nPhysics::iConstraint* sliderConstraint = physicsFactory->createSliderConstraint(sliderDef);
			physicsWorld->addConstraint(sliderConstraint);
		}
		else if (constraintType == "coneTwist") {

			nPhysics::sConeTwistConstraintDef twistDef;
			...
			// Get the rigid bodies
			twistDef.rigidBodyA = pScene->findObjectByFriendlyName(rbAName)->getComponent<cRigidBodyComponent>()->getRigidBody();
			twistDef.rigidBodyB = pScene->findObjectByFriendlyName(rbBName)->getComponent<cRigidBodyComponent>()->getRigidBody();
			...
			// Set the rest of the def and create the constraint
			twistDef.frameAEuler = frameA;
			twistDef.frameBEuler = frameB;

			nPhysics::iConstraint* twistConstraint = physicsFactory->createConeTwistConstraint(twistDef);
			physicsWorld->addConstraint(twistConstraint);
		}
		else if (constraintType == "fixed") {

			nPhysics::sFixedConstraintDef fixedDef;
			...
			// Get the rigid bodies
			fixedDef.rigidBodyA = pScene->findObjectByFriendlyName(rbAName)->getComponent<cRigidBodyComponent>()->getRigidBody();
			fixedDef.rigidBodyB = pScene->findObjectByFriendlyName(rbBName)->getComponent<cRigidBodyComponent>()->getRigidBody();
			...
			// Set the rest of the def and create the constraint
			fixedDef.pivotInA = pivotA;
			fixedDef.pivotInB = pivotB;

			nPhysics::iConstraint* fixedConstraint = physicsFactory->createFixedConstraint(fixedDef);
			physicsWorld->addConstraint(fixedConstraint);
		}
	}
...
void cSerialization::mCreateRigidBodyComponent(cEntity* entity, rapidjson::Value::ConstValueIterator& compItr) {
	...
	if (type == "sphere") {
		...
		assert((*compItr)["shape"].HasMember("halfLengths"));
		assert((*compItr)["shape"]["halfLengths"].IsObject());

		glm::vec3 halfLength(0.0f);

		assert((*compItr)["shape"]["halfLengths"].HasMember("x"));
		assert((*compItr)["shape"]["halfLengths"]["x"].IsDouble());
		halfLength.x = (*compItr)["shape"]["halfLengths"]["x"].GetDouble();

		assert((*compItr)["shape"]["halfLengths"].HasMember("y"));
		assert((*compItr)["shape"]["halfLengths"]["y"].IsDouble());
		halfLength.y = (*compItr)["shape"]["halfLengths"]["y"].GetDouble();

		assert((*compItr)["shape"]["halfLengths"].HasMember("z"));
		assert((*compItr)["shape"]["halfLengths"]["z"].IsDouble());
		halfLength.z = (*compItr)["shape"]["halfLengths"]["z"].GetDouble();

		pShape = pEngine->getPhysicsFactory()->createBoxShape(halfLength);
	}
	else if (type == "cone") {
		...
		assert((*compItr)["shape"].HasMember("height"));
		assert((*compItr)["shape"]["height"].IsDouble());
		float height = (*compItr)["shape"]["height"].GetDouble();

		pShape = pEngine->getPhysicsFactory()->createConeShape(radius, height);
	}
		...
		pShape = pEngine->getPhysicsFactory()->createCapsuleShape(radius, height);
	}
		...
		pShape = pEngine->getPhysicsFactory()->createCylinderShape(halfLength);
	}
